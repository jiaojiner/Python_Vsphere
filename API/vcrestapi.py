...
disable_warnings()  # 关闭证书错误告警

http = PoolManager()

user_pass_str = username + ':' + password  # http头部格式
user_pass_str_encode = user_pass_str.encode()  # 需要base64转码
...
def get_token(vcip, username, password):  # 获取token
...
def get_vms(vcip, token):  # 获取vm的唯一码
...
def get_hosts(vcip, token):  # 获取主机的唯一码
    headers = {'vmware-api-session-id': token}
...
def get_folders(vcip, token):  # 获取目录的唯一码
    headers = {'vmware-api-session-id': token}
...
def get_datastores(vcip, token):  # 获取存储的唯一码
    headers = {'vmware-api-session-id': token}
...
def get_networks(vcip, token):  # 获取网络的唯一码
    headers = {'vmware-api-session-id': token}
...
def create_vm(vcip, token, vmname):  # 创建虚拟机
...
def add_vm_nic(vcip, token, vmid, network_name):  # 创建网卡
    headers = {'vmware-api-session-id': token, 'Content-Type': 'application/json'}
    add_nic_json = {
        "spec": {
            "backing": {
...
def get_vm_nics(vcip, token, vmid):  # 获取网卡信息
    headers = {'vmware-api-session-id': token}
    url = 'https://' + vcip + '/rest/vcenter/vm/' + vmid + '/hardware/ethernet'
    r = http.request('GET', url, headers=headers)
    return json.loads(r.data.decode())['value']
...
def get_vm_nic_detail(vcip, token, vmid, nic):  # 获取某一个网卡的详细信息
    headers = {'vmware-api-session-id': token}
...
def update_vm_nic(vcip, token, vmid, nic, network_name):  # 更新网卡配置
    headers = {'vmware-api-session-id': token, 'Content-Type': 'application/json'}
    update_nic_json = {
        "spec": {
            "backing": {
